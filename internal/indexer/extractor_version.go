...
var extractorVersions = map[string]int{
	...
	"csharp": 22,                                     // combo lane-7b: verbatim canon (19) + repeated partial bases / chained-awaited receivers (#735) + switch-section pattern scoping (#734) + indexer/event member emission (#738) + ownership-span stamps incl. the Razor rebase (issue #731 fix); one bump per lane build so the lane store re-extracts once (was: verbatim-identifier canonicalization unified across type refs, the base-list prescan, and partial identity)
